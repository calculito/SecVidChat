...
function chatRoomFull() {
  alert(
    "The room is full. Check to make sure you don't have multiple open labels, or try using a new meeting room link."
  );
  //退出房间并重定向
...
function swap() {
  ...
  if (!WebRTC.connected) {
    alert("You must join the call before you can share the screen");
    return;
  }
  ...
  if (
    ...
    if (document.pictureInPictureElement) {
      ...
      remoteVideoGlobal.requestPictureInPicture().catch((error) => {
        alert("You must connect to someone else to enter drawing mode");
      });
    }
  } else {
    alert(
      "Your browser doesn't support drawing-in-picture. Consider using Chrome or Safari."
    );
  }
}
//Picture in picture
...
function requestPassword() {
  const sessionPassword = sessionStorage.getItem("SecVidCh");
  if (!sessionPassword) {
    const promptPassword = prompt("Please enter password", "");
    if (promptPassword != null && promptPassword != "") {
      sessionStorage.setItem("SecVidCh", promptPassword);
      password = promptPassword;
    } else {
      const promptPasswordAgain = prompt("Please enter password...", "");
    }
  } else {
...
function bootstrap() {
  ...
  //在开始时设置字幕
  captionText.textContent = "Waiting for another user to join...";
  fadeIn(captionText);
